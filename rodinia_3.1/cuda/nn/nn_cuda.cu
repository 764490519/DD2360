...
int main(int argc, char* argv[])
{
  ...
  int numStreams = 10;
  int blockSize = numRecords / numStreams;
  ...
  for (int i = 0; i < numStreams; ++i) {
	...
	*/
    ...
    cputimer_stop("Allocate memory on host and device ");
    ...
    for (int i = 0; i < numStreams; ++i) {
      ...
      find_min<<< grid_min , threadsPerBlock, 0, streams[i] >>>(d_distances + i * blockSize, numRecordsForThisBlock, d_minLoc + i * resultsCount * ((int)(sqrt((float)numRecords/(float)resultsCount)+1) + threadsPerBlock - 1) , offset, resultsCount, tmp);
    ...
    find_min_final<<< 1, 1 >>>(d_distances, (int)(sqrt((float)numRecords/(float)resultsCount)+1) * resultsCount * numStreams , d_minLoc, resultsCount, min_record_dis, tmp_final_float, d_minmem);
